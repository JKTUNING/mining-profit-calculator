...
async function getFluxTX(address,filename) {
    ...
    try {
        const response = await axios.get(
            `https://api.runonflux.io/explorer/transactions?address=${address}`,
            {
                timeout: 2500
            }
        );
...
async function fluxTX(address,fluxFileName) {
    ...
    for (let i = 0; i < flux_tx_data.length; i++) {
        ...
        const tx_response = await axios.get(
            `https://explorer.runonflux.io/api/tx/${flux_tx_data[i]}`,
            {
                timeout: 5000
            }            
        );
...
async function getFluxPrice(){
	const flux_price = await axios.get(`https://www.coingecko.com/price_charts/5163/usd/custom.json?from=1641081600&to=1672462800`);
	return flux_price?.data?.stats
}
